...
Attains 94.07 mean accuracy (n=200 trials)
"""
...
def train_loops(run, model_trainbias, model_freezebias, lr, momentum, compute_UV):
    ...
    parameters = [
        ...
        dict(norm='SpectralConv', norm_kwargs={'steps': 9}, scale=radius*2.0, params=[p for n, p in model.named_parameters() if len(p.shape) == 4 and p.requires_grad and "0.weight" not in n]),
        dict(norm='BiasRMS', scale=radius/2.0, params=[p for n, p in model.named_parameters() if 'norm' in n and p.requires_grad]),
        dict(norm='Sign', scale=head_radius, params=[fc_layer]),
    ]
    optimizer = Scion(parameters, lr=lr, momentum=momentum, compute_UV=compute_UV)
    optimizer_trainbias = optimizer
    optimizer2_trainbias = Scion(norm='BiasRMS', scale=radius, params=[whiten_bias], lr=lr, momentum=momentum, compute_UV=compute_UV)
...
def scion_cifar10(compute_UV):
    ...
    for width_factor in [1.0]:
        ...
        train_loops('warmup', model_trainbias, model_freezebias, lr=0.05, momentum=0.5, compute_UV=compute_UV)
        
        #for log2lr in np.linspace(-9, 0, 10):
        for log2lr in [math.log2(0.04)]:
            momentum = 0.65

            accs = torch.tensor([train_loops(run, model_trainbias, model_freezebias, lr=2**log2lr, momentum=momentum, compute_UV=compute_UV) for run in range(3)])
