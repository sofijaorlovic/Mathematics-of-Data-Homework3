...
class SpectralConv(Norm):
    def __init__(self, steps=5, compute_UV = None):
        self.steps = steps
        self.compute_UV = compute_UV

    def lmo(self, g):
        g = self.compute_UV(g.reshape(len(g), -1), steps=self.steps).view(g.shape)
...
class Spectral(Norm):
    def __init__(self, max=False, normalized=True, steps=5, compute_UV=None):
        self.max = max
        self.steps = steps
        self.normalized = normalized
        self.compute_UV = compute_UV
...
class Scion(torch.optim.Optimizer):
    ...
    """
    def __init__(self, params, lr=1e-3, momentum=1.0, norm: str='Auto', norm_kwargs: dict=None, scale=1.0, unconstrained=False, compute_UV = None):
        if lr < 0.0:
    ...
    def step(self):
        for group in self.param_groups:
            ...
            if "Spectral" in group['norm']:
                group['norm_kwargs']['compute_UV'] = self.compute_UV
